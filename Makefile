binaryen-debug: ./binaryen/src/binaryen-c.h
	cd binaryen && emcmake cmake -DBUILD_STATIC_LIB=ON -DBUILD_TOOLS=OFF -DBUILD_TESTS=OFF -DENABLE_BIGINT=ON -DCMAKE_BUILD_TYPE=Debug . && emmake make binaryen

binaryen-release: ./binaryen/src/binaryen-c.h
	cd binaryen && emcmake cmake -DBUILD_STATIC_LIB=ON -DBUILD_TOOLS=OFF -DBUILD_TESTS=OFF -DENABLE_BIGINT=ON -DCMAKE_BUILD_TYPE=Release . && emmake make binaryen
